...
/** Horizontal run flick poster. Axis +1 is right, −1 is left. */
export class RunHint extends LevelHint {
	...
	protected buildTouchTimeline(): gsap.core.Timeline {
		...
		const slideSec = Math.abs(endX - startX) / HINT_JUMP_SPEED;
		...
		timeline.to(this.tip, { x: endX, duration: slideSec, ease: 'power2.in' });
		timeline.call(() => {
			this.setSamplingTrail(false);
		});
