...
- Platform plates (`ground` / `leaf` / `wall` / `sticky`) are the ship look — translucent rounded Graphics, not placeholders. Optional later: tint / alpha only (no painted leaf textures). **Physics** for those plates is a sharp AABB; the “rounded ice” feel is the circle blob (r = 30) + player `friction: 0`. Spikes are a vector saw (drawn AABB; killbox inset 6 px; optional later: round the inner core rectangle). Stones / branches are opaque Graphics (circle / chamfered log). Droplet splash is in (even fan); puddle / land polish deferred.
...
| 2 | Spikes / death surfaces | **Done.** Level `hazards[]` (`type: spikes`); solid Matter `hazard` AABB. Killbox inset **6 px** from the drawn box (art unchanged). Display: generated isosceles saw inside that box ([`src/entities/spike-outline.ts`](../src/entities/spike-outline.ts)); optional `facing` + `length`. Not debug rects / not painted spike art. Optional later: round the inner core rectangle. Shared death pipeline with fall (`beginDeath` → `burst` anim → pause → respawn). Death VFX: pooled kinematic droplets (`src/fx/blob-droplet-pool.ts`), even circular fan. Burst sheet may be a blank frame (droplets carry the pop); last frame is not force-hidden. **Moving insects (E3) done:** `caterpillar` / `spider` / `mosquito` reuse this kill path (sensor AABB on a `from`–`to` rail). Demo placement: caterpillar `meadow-04`, spider `meadow-09`, mosquito `bonus-level`. |
...
| — | Movable obstacles | **Done (runtime, feel still open).** Level `obstacles[]` (`stone` / `branch`); Matter dynamic, walkable, not lethal. Stone `size` = diameter (**Ogmo `width` → `size`**; ignore origin). Branch: Ogmo vertical rect + **`rotation` in radians** (0 = down); authoring angle `-90 - deg(rotation)`; loader Y-flip then negate. Fall-cull same slack as the blob. Spikes solid for them; fireflies / portal / insects ignored. Demo set: stone `meadow-03` / `bonus-level`, branch `meadow-06`. Sandbox JSON (`meadow-13`, `testlevel-00`) is not in the carousel. **Why a shove ignores density:** blob `setVelocity` each frame + `friction: 0` (pair friction with the blob is 0). Obstacle `friction` / `frictionStatic` still matter vs platforms. Branch tip-over is also `inertia` (∝ mass × length²) + `frictionAir` (damps ω) + chamfered foot (skates, not a pin). **Tried:** stop overwriting `vx` while pushing — rolled back (no walk on inclines; props on the head went “balloon”). **Now:** ½ `MOVE_SPEED_X` if a SAT side-probe hits an `obstacle` (not `platform`); probe is a thin rect, not the branch AABB (AABB was slowing both directions). Full force-based blob deferred. Tunables: `SIDE_PROBE_*` / `SIDE_OBSTACLE_SPEED_SCALE` in `player.ts`; density/friction in `obstacle.ts`. |
