...
| 3 | Crouch / hide | **Done.** Hold ↓ / `KeyS` / touch bottom-center; blend-in squat (½ height, breath ×¼, alpha 0.6); Matter collider scaleY → 0.5 (feet planted); stand still (no crawl); release → micro-hop stand; jump from any crouch skips squat wind-up and uses `CROUCH_JUMP_VELOCITY` (−14) with a lower-pitched jump SFX (`speed` ≈ 0.7–0.8); `isHidden` at full blend for future LOS. |
